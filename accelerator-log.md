...
```json
{
  ...
  "projectName" : "asa-weatherforecast"
}
```
## Log
```
...
┃ ┃ ┃ Debug Adding symbol compilerSafeAppName with value 'weather'
...
┃ ┃ ┃ ┃ ┃ ┗ ┗  Info Will replace [Sample->weather]
...
┃ ┃ ┃ ┃ ┃ ┗ ┗ Debug Path 'Sample.csproj' matched '^(?<folder>.*/)?(?<filename>([^/]+?|)(?=(?<ext>\.[^/.]*)?)$)' with groups {ext=.csproj, folder=null, filename=Sample.csproj, g0=Sample.csproj, g1=null, g2=Sample.csproj, g3=Sample.csproj, g4=.csproj} and was rewritten to 'weather.csproj'
...
┃ ┃ ┃ ┃ ┃ ┗ ┗  Info Will replace [sample-app->asa-weatherforecast]
